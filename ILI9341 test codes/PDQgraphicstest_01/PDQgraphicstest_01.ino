...
void setup()
{
  ...
  // (2) 디스플레이 초기화 (Init Display)
  // Simple Usage에 따라 gfx->begin()을 호출합니다 [1].
  if (!gfx->begin())
  ...
  // (3) 화면 초기화 (추가된 부분)
  // 디스플레이 초기화가 성공적으로 이루어진 후, 화면을 검은색으로 채웁니다 [1].
  // RGB565는 16비트 컬러를 나타내는 데 사용됩니다 [2].
  gfx->fillScreen(RGB565_BLACK);

  // (4) 디스플레이 치수 계산 및 테스트 변수 설정
  w = gfx->width();
  h = gfx->height();
  n = min(w, h);
  n1 = n - 1;
